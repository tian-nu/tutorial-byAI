...
接下来的操作全部在你的 `tian-nu/test` 仓库中进行。因为是个人项目，所以你是"开发者"也是"审查者"——自己给自己发 PR、自己审查自己、自己合并。听起来像自言自语，但流程和团队协作完全一样。
...
remote:
To git@github.com:tian-nu/test.git
...
先**按 F5 键**刷新你的仓库首页（`github.com/tian-nu/test`）。刷新后，注意看页面顶部，有没有出现一个**黄色的横条**，上面写着类似的话：
...
```
https://github.com/tian-nu/test/pull/1
```
...
- 浏览器地址栏显示 `github.com/tian-nu/test/pull/1`（或 /pull/2、/3...）
- 页面顶部显示你的 PR 标题
...
你该看到类似输出：
```
From git@github.com:tian-nu/test
