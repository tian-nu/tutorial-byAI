...
1. **使用轻量基础镜像**：`alpine`版本比完整版小10倍
2. **合并RUN指令**：减少镜像层数
3. **用多阶段构建**：编译在一个阶段，运行在另一个阶段
4. **COPY比ADD优先**：COPY更透明，ADD有隐式行为
5. **用.dockerignore**：排除不必要的文件
...
---
[← 上一章：附录C-Redis命令速查.md](附录C-Redis命令速查.md) | [下一章：附录E-Git命令速查.md →](附录E-Git命令速查.md)
