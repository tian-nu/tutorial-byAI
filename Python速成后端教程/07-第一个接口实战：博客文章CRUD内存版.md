...
- 前置教程：[06-请求体：POST 请求](./06-请求体：POST请求.md)
- 可视化：无
...
- **下一章**：[08-中间件：窥探每个请求](./08-中间件：窥探每个请求.md)——学习中间件，看看每个请求进来时服务器到底发生了什么。
...
> [可暂停点 4/9]：阶段二即将结束。你已经能写完整的 CRUD 接口了（虽然数据还存在内存里）。下次从 [08-中间件：窥探每个请求](./08-中间件：窥探每个请求.md) 继续。