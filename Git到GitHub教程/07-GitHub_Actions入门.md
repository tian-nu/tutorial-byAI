...
我们用的 `tian-nu/test` 是公开仓库，所以完全免费。
...
2. 在文件资源管理器的左侧或顶部地址栏里，找到你的项目文件夹 `my-forked-project`。如果你把它放在了桌面，路径大概是：
   ```
   C:\Users\<你的用户名>\Desktop\my-forked-project
   ```
   双击 `my-forked-project` 文件夹，进入它。
...
现在你有了一条路径：`my-forked-project/.github/workflows/`。

### 第2步：在 workflows 文件夹里创建 YAML 文件

1. 确保你现在在 `workflows` 文件夹里（文件夹路径应该类似于 `...\my-forked-project\.github\workflows`）
...
回到你的**终端**（黑底白字窗口），确保你在 `my-forked-project` 文件夹里：
...
1. 打开你的 Fork 仓库页面 → 点击仓库顶部 **"Code"** 标签 → 在文件列表中你应该能看到 `.github` 文件夹（注意前面有个点）
